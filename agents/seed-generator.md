...
**MANDATORY:** Before declaring seed data complete, invoke `superpowers:verification-before-completion` via the Skill tool. Run the seed script and confirm it executes without errors. Verify referential integrity — every foreign key must point to a row that exists. Show the row counts after seeding.
