...
**MANDATORY:** Before running any manual grep-based checks, invoke the following skills via the Skill tool:

1. `semgrep` — run static analysis with parallel subagents across the full codebase. Use "important only" mode for routine audits, "run all" for pre-release.
2. `insecure-defaults` — detect hardcoded secrets, weak auth configs, and fail-open patterns.
3. `codeql` — for penetration testing mode or pre-release audits. Finds injection paths, auth bypasses, and data exposure chains via interprocedural data flow. Pipe results through `sarif-parsing` to merge with semgrep output.

The grep patterns below supplement automated scanning — they are not a substitute for it. Consolidate all findings (automated + manual) into a single report. Use `sarif-parsing` to merge SARIF outputs if both semgrep and codeql have been run.
...
## 8. Penetration Testing Mode

When performing active security assessment rather than passive audit, follow PTES phases:

```
1. PRE-ENGAGEMENT    — Define scope, rules of engagement, written authorization
2. RECONNAISSANCE    — Passive (OSINT, subfinder, Shodan) → Active (nmap)
3. THREAT MODELING  — Map attack surface and prioritize vectors
4. VULN ANALYSIS    — Discover and validate weaknesses (Burp Suite, ZAP, sqlmap)
5. EXPLOITATION     — Demonstrate impact (Metasploit, manual exploitation)
6. POST-EXPLOITATION — Privilege escalation, lateral movement (BloodHound)
7. REPORTING        — Document findings with evidence and CVSS scores
```

**2025 OWASP Top 10 (updated categories):**

| # | Vulnerability | New in 2025 |
|---|---------------|-------------|
...
**Risk prioritization:** `Risk = Likelihood × Impact`. High+High = CRITICAL. Patch in this order.

**Ethical hard limits:** Never access out-of-scope systems. Never exfiltrate real user data. Never exploit without written authorization.
