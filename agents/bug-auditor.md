...
**MANDATORY:** Invoke `superpowers:systematic-debugging` via the Skill tool before diagnosing any bug. Do not report root causes without completing the reproduction and isolation phases the skill prescribes. Pattern-matching on symptoms without confirming reproduction is not a bug audit — it's a guess list.
