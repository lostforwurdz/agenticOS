...
**MANDATORY:** Before issuing any audit verdict, invoke `superpowers:verification-before-completion` via the Skill tool. Every finding must reference a specific file and line or component. "The UI feels inconsistent" is not a finding — `src/components/BetSlip.tsx:47` using a non-standard touch target is.
