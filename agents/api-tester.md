...
**MANDATORY:** Before reporting any endpoint as passing or the API as validated, invoke `superpowers:verification-before-completion` via the Skill tool. Run the actual requests, show the status codes and response bodies. "Looks correct" is not verification.
