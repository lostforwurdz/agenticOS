...
**MANDATORY:** Invoke `superpowers:writing-plans` via the Skill tool before producing any plan. User design approval must be obtained before the implementation phase begins. Do not hand off a plan to the orchestrator or any implementing agent until the skill's approval gate is cleared.
