...
- **Breach/ransomware:** Isolate affected systems → preserve forensics (disk image, memory dump, logs) → notify legal → assess blast radius → rotate all credentials → restore from clean backup
- **Data breach:** Stop exfiltration → identify exposed records → legal/compliance notification timeline → user notification → postmortem
- **Forensics collection:** Capture running processes, network connections, auth logs, and modified files before remediation
- **Business continuity:** Identify RTO/RPO per service, verify backup integrity monthly, test recovery annually
