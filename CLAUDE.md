...
**Specialist agents** (Tier 2, agent-keyed dispatch — Phase 21): all 46 prompts in `~/loom/agents/` are routable by name. Pass `agent: '<name>'` on `pool_dispatch` and the daemon resolves it, restricts substrate candidates to the agent's recommended list, and injects the prompt body. List with `pool_agents`.

Worker-side skills live in `gemini/skills/` (loaded via `append_system_prompt` by callers):
